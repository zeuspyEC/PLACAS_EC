...
def admin_security():
    ...
    # Log de acceso admin
    logger.warning(
        f"Admin access attempt: {request.method} {request.path} from {request.remote_addr}"
    )
...
def system_info():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "error": "Error obteniendo información del sistema"}
            ),
            500,
        )
...
def database_info():
    ...
    try:
        ...
        if not db_path or not Path(db_path).exists():
            return (
                jsonify({"success": False, "error": "Base de datos no encontrada"}),
                404,
            )
...
def list_logs():
    ...
    try:
        ...
        if not logs_dir.exists():
            return (
                jsonify(
                    {"success": False, "error": "Directorio de logs no encontrado"}
                ),
                404,
            )
        ...
        return (
            jsonify({"success": False, "error": "Error listando archivos de log"}),
            500,
        )
...
def view_log(log_name):
    ...
    try:
        ...
        try:
            ...
            return jsonify({"success": False, "error": "Ruta de archivo inválida"}), 400
        ...
        return jsonify({"success": False, "error": "Error leyendo archivo de log"}), 500
...
def cache_status():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": "Error obteniendo estado del cache"}),
            500,
        )
...
def list_backups():
    ...
    try:
        ...
        if not db_path:
            return (
                jsonify(
                    {"success": False, "error": "Ruta de base de datos no configurada"}
                ),
                500,
            )
        ...
        return jsonify({"success": False, "error": "Error listando backups"}), 500
...
def view_config():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "error": "Error obteniendo configuración"}),
            500,
        )
...
def performance_metrics():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": False, "error": "Error obteniendo métricas de performance"}
            ),
            500,
        )
